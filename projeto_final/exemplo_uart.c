/*
*  Descontinuado por  tensão diferente entre a rp2040(3.3v) e a placa nano(5v) 
* Este exemplo demonstra como configurar uma UART para ler dados de um sensor externo e integrar
* essa leitura com o sistema de eventos do FreeRTOS. O código inclui a configuração da UART, uma
* tarefa dedicada para ler os dados, e o envio de eventos para outras partes do sistema (como UI e 
* HTTP) com base nas leituras da UART. 
*/
...
// UART para sensor extra
#define UART_EXT uart1
#define UART_EXT_TX_PIN 2   // GP2
#define UART_EXT_RX_PIN 3   // GP3
...
// setup da UART para sensor extra
void setup_uart_ext() {
    uart_init(UART_EXT, UART_BAUDRATE);

    gpio_set_function(UART_EXT_TX_PIN, GPIO_FUNC_UART);
    gpio_set_function(UART_EXT_RX_PIN, GPIO_FUNC_UART);

    uart_set_format(UART_EXT, 8, 1, UART_PARITY_NONE);
    uart_set_fifo_enabled(UART_EXT, true);
}
...
// Tarefa UART - Responsável por ler dados de um sensor extra via UART
void task_uart_ext(void *pvParameters) {
    char buffer[32];
    int idx = 0;

    for (;;) {

        while (uart_is_readable(UART_EXT)) {
            char c = uart_getc(UART_EXT);

            if (c == '\n' || idx >= sizeof(buffer) - 1) {
                buffer[idx] = '\0';
                idx = 0;

                if (strcmp(buffer, "VISITA") == 0) {
...
int main(){
    ...
    setup_uart_ext(); // UART para sensor extra
    ...
    xTaskCreate(task_uart_ext, "UART_EXT", 1024, NULL, tskIDLE_PRIORITY + 2, NULL);
